...
def main():
    ...
    top_column = [
        ...
        sg.Frame('',[
                ...
                [
                    ...
                    sg.Checkbox(MAKE_SAFETENSORS_LBL,k=MAKE_SAFETENSORS_CHKBOX_KEY,default=True,enable_events=True,font=FONT),
    ...
    def merge_models_thread(model_a:str, model_b:str, device:str="cpu", output:str="merged", make_safetensors:bool=True, usefp16:bool=True, alpha:float=0.5, iterations:int=10):

        def load_model(model, device):
            if os.path.splitext(model)[1] == '.safetensors':
                return safetensors.torch.load_file(model, device=device)
            else:
                return torch.load(model, map_location=device)["state_dict"]

        def merge_models(model_a:str, model_b:str, device:str="cpu", output:str="merged", make_safetensors:bool=True, usefp16:bool=True, alpha:float=0.5, iterations:int=10):
            print(f"""
            ---------------------
                ...
                alpha:      {alpha}
                usefp16:    {usefp16}
                safetensor: {make_safetensors}
                iterations: {iterations}
            ---------------------
            """)
            model_a = load_model(model_a, device)
            model_b = load_model(model_b, device)
            theta_0 = model_a
            theta_1 = model_b
            ...
            for x in range(iterations):
                ...
                first_permutation, y = weight_matching(permutation_spec, model_a, theta_0, usefp16=usefp16)
                theta_0 = apply_permutation(permutation_spec, first_permutation, theta_0)
                second_permutation, z = weight_matching(permutation_spec, model_b, theta_0, usefp16=usefp16)
        ...
        try:
            merge_models(model_a=model_a, model_b=model_b, device=device, output=output, make_safetensors=make_safetensors, usefp16=usefp16, alpha=alpha, iterations=iterations)
    ...
    def merge_modelname(model_a:str, model_b:str, make_safetensors:bool=True, usefp16:bool=True, alpha:float=0.1, iterations:int=10)-> str:
        alpha_b = 1 - (float(alpha))
        alpha_b = round(alpha_b, 2)
        fp16 = "_fp16" if usefp16 else ""
        fileformat = "safetensors" if make_safetensors else "ckpt"
        output = f"{os.path.dirname(model_a)}/{os.path.splitext(os.path.basename(model_a))[0]}_{alpha}_{os.path.splitext(os.path.basename(model_b))[0]}_{alpha_b}_{iterations}it{fp16}.{fileformat}"
        return output
    ...
    while True:
        ...
        if event == MERGE_MODELS_BTN_KEY:
            ...
            make_safetensors = values[MAKE_SAFETENSORS_CHKBOX_KEY]
            ...
            if model_a and model_b and output:
                ...
                Thread(target=merge_models_thread, args=(model_a, model_b, device, output, make_safetensors, usefp16, alpha, iterations), daemon=True).start()
            else:
